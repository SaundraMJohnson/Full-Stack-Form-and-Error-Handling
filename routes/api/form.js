...
router.post('/', (req, res, next) => {
    ...
    if(!firstName || !lastName || !email || !occupation || !city || !bio) return res.json({msg: 'Please fill in all fields'});
    Form.create(req.body, (err, newdata) => {
        if(err) return res.json({err});
        return res.json({success: true, newdata, msg: "Thanks for getting in touch, We'll get back soon"});
    });
})
